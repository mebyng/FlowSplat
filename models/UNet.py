...
class RotationConditionedUNet(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3, base_channels=32, cond_dim=64):
        ...
        self.bottleneck = _ConvBlock(base_channels * 4, base_channels * 8, cond_dim=cond_dim)
    ...
    def forward(self, image, rotation, timestep):
        """Forward pass.
        ...
        cond = torch.cat([rotation.view(batch_size, 9), timestep], dim=1)
        cond = self.cond_embed(cond)

        x1 = self.enc(image, cond=cond) # shape (B, base_channels, H, W)
        x2 = self.down1(x1, cond=cond) # shape (B, base_channels*2, H/2, W/2)
        x3 = self.down2(x2, cond=cond) # shape (B, base_channels*4, H/4, W/4)

        x = self.bottleneck(x3, cond=cond) # shape (B, base_channels*8, H/4, W/4)
        x = self.up1(x, x2, cond=cond) # shape (B, base_channels*4, H/2, W/2)
        x = self.up2(x, x1, cond=cond) # shape (B, base_channels*2, H, W)
        x = self.dec(x, cond=cond) # shape (B, base_channels, H, W)
